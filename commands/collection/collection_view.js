...
module.exports = {
  async execute(interaction) {
    ...
    const generateEmbed = (page) => {
      ...
      items.forEach((item) => {
        const userItem = userInventory.items.find(
          (userItem) => userItem.name === item.name
        );

        if (userItem) {
          embed.addFields({
            name: ` ${userItem.emoji} ${userItem.name}`,
            value: `Rareté: ${userItem.rarity}\nPrix: ${userItem.price} <:gold:1261787387395047424>\n**(Possédé x${userItem.quantity})**`,
            inline: true,
          });
        } else {
          embed.addFields({
            name: ` ${item.emoji} ${item.name}`,
            value: `Rareté: ${item.rarity}\nPrix: ${item.price} <:gold:1261787387395047424>\n*(Non possédé)*`,
            inline: true,
          });
        }
      });
